...
export const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_SERVER_URL || 'http://localhost:3001'}/auth/login`, {
        method: 'POST',
  ...
  return (
    <div className="w-screen h-screen flex items-center justify-center"
      ...
      <div className="w-full max-w-md px-6 z-10">
        <div className="bg-gradient-to-br from-[#1a2a4a] to-[#0d1f3c] border border-gold/30 rounded-2xl p-8 shadow-[0_24px_80px_rgba(0,0,0,0.8)]">
          <div className="text-center mb-6">
            <h1 className="font-display text-3xl text-gold-light mb-2 tracking-tight">Welcome Back</h1>
            <p className="text-foreground/40 text-xs font-bold uppercase tracking-[3px]">Sign in to continue</p>
          </div>
          
          {error && (
            <div className="mb-5 p-3 rounded-xl bg-destructive/20 border border-destructive/40 text-destructive-200 text-xs flex items-center gap-2">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <input
                ...
                required
                placeholder="commander@lastcard.com"
              />
            </div>
            <div className="space-y-1.5">
            ...
            >
              {loading ? <Loader2 className="animate-spin" size={18} /> : <><ArrowRight size={18} /> Enter Arena</>}
            </button>
          </form>
          ...
          <p className="mt-6 text-center">
            <span className="text-foreground/40 text-[10px] font-bold uppercase tracking-widest">New Player? </span>
            <Link to="/signup" className="text-gold text-[10px] font-bold uppercase tracking-widest hover:underline ml-1">Create Account</Link>
          </p>
        </div>
      </div>
    </div>
